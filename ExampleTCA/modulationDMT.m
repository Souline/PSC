...
% table : tableau de int ( table d'allocation des bits )
    ...
    for i=1:(nb_canaux)                                                          % modulation de chaque sous canal                                                                 % parcours des canaux
        if(table(i) ~= 0)      
            suite_symboles(i) = bin2dec(num2str(suite_bits(index:index+table(i)-1), '%1d'));    % création de nos symboles QAM
            coefficients(i) = qammod(suite_symboles(i),2^(table(i)));                           % Modulation QAM
            qref = qammod([0:2^(table(i))-1],2^(table(i)));
            correction_qam = modnorm(qref,'avpow',table(i)*puissance_par_bit);   % coefficients de normalisation
            coefficients(i)=coefficients(i)*correction_qam;                      % normalisation et adaptation de puissance
            index=index+table(i);                                                % extraction des bits suites
        else
            coefficients(i) = 0;                                                   
        end
    end
    ...
    HF = [coefficients(1:256) 0 conj(fliplr(coefficients(2:256))) ];        % Symétrie hermitienne
