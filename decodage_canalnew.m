...
%
trame2 = deinterleaver( trame, 3, 2 );
taille_trame = length( trame2 );

% RS
N_check = floor( taille_trame / ( 8 * 224 ) );
N_rs = floor( taille_trame / ( 8 * 240 ) );
trame_rs_end = trame2( ( N_rs*8*240+1 ):taille_trame );
...
if N_check > 0
  for i = 1:N_rs
    encoded = rs_decoding( trame2((i-1)*8*240+1:i*8*240), 240, 224 );
